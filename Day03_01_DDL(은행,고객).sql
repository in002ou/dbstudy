/*
    ...
    2. 데이터 정의어
    3. 데이터베이스 객체(USER, TABLE, SEQUENCE, VIEW, INDEX 등)를 생성/수정/삭제하는 언어.
...
-- 1. BANK_TBL 테이블에 연락처(BANK_TELL) 칼럼 추가
ALTER TABLE BANK_TBL
    ADD BANK_TELL VARCHAR2(20 BYTE) NOT NULL;

-- 2. CUSTOMER_TBL 테이블에서 나이(AGE) 칼럼 삭제
ALTER TABLE CUSTOMER_TBL
    DROP COLUMN AGE;

-- 3. BANK_TBL 테이블의 은행명(BANK_NAME) 칼럼의 데이터타입 VARCHAR2(15 BYTE)으로 변경.
ALTER TABLE BANK_TBL
    MODIFY BANK_NAME VARCHAR2(15 BYTE);

-- 4. CUSTOMER_TBL 테이블에서 고객명(NAME) 칼럼의 이름을 CUST_NAME으로 변경
ALTER TABLE CUSTOMER_TBL
    RENAME COLUMN NAME TO CUST_NAME;

-- 개인 실습
-- 5. CUSTOMER_TBL 테이블에 GRADE 칼럼 추가
--  GRADE 칼럼은 ('VIP', 'GOLD', 'SILVER', 'BRONZE' 중 하나의 값만 가질 수 있도록 CHECK 제약조건을 지정
ALTER TABLE CUSTOMER_TBL
    ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE IN('VIP', 'GOLD', 'SILVER', 'BRONZE'));
    
-- 6. BANK_TBL 테이블의 BANK_NAME 칼럼에 NOT NULL 제약조건 추가
ALTER TABLE BANK_TBL
    MODIFY BANK_NAME VARCHAR2(15 BYTE) NOT NULL;
    
-- 7. CUSTOMER_TBL 테이블의 NO 칼럼의 이름을 CUST_NO으로 변경
ALTER TABLE CUSTOMER_TBL
    RENAME COLUMN NO TO CUST_NO;
    
-- 8. CUSTOMER_TBL 테이블의 PHONE 칼럼 삭제
ALTER TABLE CUSTOMER_TBL
    DROP COLUMN PHONE;
    
-- 9. CUSTOMER_TBL 테이블의 CUST_NAME 칼럼의 NOT NULL 제약조건을 NULL 제약조건으로 변경
ALTER TABLE CUSTOMER_TBL
    MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL;    -- NULL은 생략이 가능하나 수정 시에 생략을 하면 수정 이전 제약조건을 사용하겠다는 뜻으로 NULL 기입해야 한다.
