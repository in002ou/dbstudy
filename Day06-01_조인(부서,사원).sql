/*
    ...
    1. 드라이브(DRIVE) 테이블
        ...
        3) 행(ROW)의 개수가 일반적으로 적고, PK를 조인 조건으로 사용하기 때문에 인덱스(INDEX) 활용이 가능
    2. 드리븐(DRIVEN) 테이블
        1) 1:M 관계에서 M에 해당하는 테이블
        2) 행(ROW)의 개수가 일반적으로 많고, FK를 조인 조건으로 사용하기 때문에 인덱스(INDEX) 활용 불가
    3. 조인 성능 향상을 위해서 가급적 드라이브(DRIVE) 테이블을 먼저 작성. 드리븐(DRIVEN) 테이블을 이후에 작성.
*/
...
-- 외래키 제약조건이 없는 상태이므로, 제약조건을 위배하는 데이터를 입력할 수 있다.(참조 무결성 위배 가능)
...
-- 3. 오른 쪽 외부 조인(오른쪽에 있는 테이블은 일치하는 정보가 없어도 무조건 조인한다.)
-- 1) 표준 문법
SELECT D.DEPT_NO, D.DEPT_NAME, E.EMP_NO, E.NAME
...
-- 외래키 제약 조건을 위반하는 데이터 삭제
DELETE FROM EMPLOYEE_TBL WHERE EMP_NO = 1005;   -- PK를 조건으로 사용하면 인덱스를 타기 때문에 빠르다.
-- DELETE FROM EMPLOYEE_TBL WHERE NAME = '김성실'; -- 인덱스를 타지 않는 일반칼럼 느림..
COMMIT;
