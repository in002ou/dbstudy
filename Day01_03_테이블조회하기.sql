/*
    ...
    5. 형식([]는 생략 가능)
        SELECT 조회할 칼럼, 조회할 칼럼, 조회할 칼럼, ... 
        FROM 테이블이름
        [WHERE 조건식]
        [GRUOP BY 그룹화할칼럼 [HAVING 그룹조건식]]
        [ORDER BY 정령할칼럼 정렬방식]
    6. 순서
        ④ SELECT 조회할 칼럼, 조회할 칼럼, 조회할 칼럼, ... 
        ① FROM 테이블이름
...
/*
    트랜잭션
    1. Transaction
    2. 여러 개의 세부 작업으로 구성된 하나의 작업을 의미
    3. 모든 세부 작업이 성공하면 COMMIT하고, 하나라도 실패하면 모든 서부 작업의 취소를 진행한다.
        (All or Nothing)
*/
...
-- 4) 테이블 별명 지정하기
SELECT E.ENAME
  FROM EMP E; -- EMP 테이블의 별명을 E로 부여. AS(ALIAS)를 사용할 수 없다.

-- 5) 칼럼 별명 지정하기
SELECT E.ENAME AS 사원명  -- E.NAME 칼럼의 별명을 '사원명'으로 부여. AS(ALIAS)를 사용할 수 있다. 
  FROM EMP E;
...
--    1) WILD CARD
--       (1) % : 모든 문자, 글자 수 제한 없는 모든 문자
--       (2) _ : 1글자로 제한된 모든 문자
--    2) 연산자
--       (1) LIKE     : WILD CARD 포함
--       (2) NOT LIKE : WILD CARD 미포함
SELECT EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO
  FROM EMP
 WHERE ENAME LIKE 'A%';
