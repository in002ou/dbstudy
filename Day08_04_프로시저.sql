...
CREATE OR REPLACE PROCEDURE BUY_PROC(CNO IN CUST_TBL.C_NO%TYPE, PCODE IN PROD_TBL.P_CODE%TYPE, BUYAMOUNT IN BUY_TBL.B_AMOUNT%TYPE)
IS
BEGIN
    -- 1) 구매내역 테이블 구매 내역을 추가(INSERT)
    INSERT INTO BUY_TBL(B_NO, C_NO, P_CODE, B_AMOUNT) VALUES(BUY_SEQ.NEXTVAL, CNO, PCODE, BUYAMOUNT);
    
    -- 2) 제품 테이블의 재고 내역을 수정(UPDATE)
    UPDATE PROD_TBL SET P_STOCK = P_STOCK - BUYAMOUNT WHERE P_CODE = PCODE;
    
    -- 3) 고객 테이블의 포인트를 수정(UPDATE)
    --    총 구매액의 10%를 정수로 올림처리해서 포인트로 준다.
    UPDATE CUST_TBL SET C_POINT = C_POINT + CEIL((SELECT P_PRICE FROM PROD_TBL WHERE P_CODE = PCODE) * BUYAMOUNT * 0.1)WHERE C_NO = CNO;
...
-- BUY_PROC 호출
EXEC BUY_PROC(1, 1000, 10);     -- 고객번호 1, 제품코드 1000, 구매수량 10
