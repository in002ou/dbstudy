...
-- 출력 파라미터
-- 1. 프로시저의 샐행 결과를 저장하는 변수
-- 2. 형식 : 변수명 OUT 타입
CREATE OR REPLACE PROCEDURE PROC4(FNAME OUT EMPLOYEES.FIRST_NAME%TYPE, LNAME OUT EMPLOYEES.LAST_NAME%TYPE)
IS
BEGIN
    SELECT FIRST_NAME, LAST_NAME
      INTO FNAME, LNAME     -- 여기서 출력 파라미터 값 저장
      FROM EMPLOYEES
     WHERE EMPLOYEE_ID = 100;
END;

-- 프로시저4 호출
-- FNAME과 LNAME값을 저장할 변수를 선언한 뒤, 프로시저에 전달
DECLARE
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
    LNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    PROC4(FNAME, LNAME);    -- PLSQL 내부에서 프로시저를 호출할 때는 EXECUTE를 생략해야한다.
...
-- PROC6(100, FNAME) : 사원번호 100은 스티븐
-- PROC6(0, FNAME)   : 사원 번호 0은 Noname
