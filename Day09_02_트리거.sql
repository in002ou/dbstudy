/*
    ...
    2. 행(ROW) 단위로 트리거가 동작.
    3. 종류
        1) BEFORE 트리거 : DML 동작 이전에 수행
        2) AFTER 트리거  : EML 동작 이후에 수행
    4. 형식
        CREATE [OR REPLACE] TRIGGER 트리거명
        BEFORE | AFTER
        INSERT OR UPDATE OR DELETE
        ON 테이블명         대상
        FOR EACH ROW    행 단위로 트리거 동작
        BEGIN
            트리거본문
        END;
*/
...
-- TRIG1 확인(EMPLOYEES 테이블의 데이터를 수정해 본다.)
UPDATE EMPLOYEES
    SET LAST_NAME = 'Queen'
...
/*
    OLD 테이블
    1. INSERT, UPDATE, DELETE 수행 이전의 정보를 임시 저장하고 있는 테이블.
    2. :OLD 방식으로 호출.
    3. AFTER 트리거와 함께 사용
        1) AFTER INSERT : NULL
...
/*
    트리거 실습
    사원 테이블에서 삭제한 사원정보를 퇴사자 테이블로 삽입하는 RETIRE_TRIG 생성하기
    
    작업 순서
    1. 사원(EMPLOYEES) 테이블의 구조를 복사하여 퇴사자(RETIRES) 테이블을 생성
    2. 퇴사자(RETIRES) 테이블에 퇴사번호 (RETIRE_ID), 퇴사일자(RETIRE_DATE) 칼럼을 추가한다.
    3. 퇴사번호(RETIRE_ID) 칼럼을 기본키로 지정
    4. 퇴사번호를 생성할 RETIRE_SEQ 시퀀스를 생성
    5. RETIRE_TRIG 트리거를 생성.
        1) AFTER DELETE 트리거
        2) OLD 테이블의 데이터를 모두 퇴사자 테이블로 삽입
        3) 퇴사번호는 시퀀스, 퇴사일자는 SYSDATE로 처리해서 삽입
*/
...
CREATE OR REPLACE TRIGGER RETIRE_TIRG
    AFTER
    DELETE
    ON EMPLOYEES
    FOR EACH ROW
BEGIN
    INSERT INTO RETIRES(RETIRE_ID, RETIRE_DATE, EMPLOYEE_ID, LAST_NAME, EMAIL, HIRE_DATE, JOB_ID)
         VALUES(RETIRE_SEQ.NEXTVAL, SYSDATE, :OLD.EMPLOYEE_ID, :OLD.LAST_NAME, :OLD.EMAIL, :OLD.HIRE_DATE, :OLD.JOB_ID);
END;
